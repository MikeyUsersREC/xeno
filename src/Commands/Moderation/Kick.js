...
module.exports = class extends Command {
    ...
    async run(message, ...args) {
        ...
        if (target_role > member_role && !message.member === message.guild.owner) {
            return message.channel.send('You cannot kick this user since they are higher than you.')
        }
        ...
        if (target === message.guild.owner) {
            return message.channel.send('You are not permitted to kick the server owner.')
        }
