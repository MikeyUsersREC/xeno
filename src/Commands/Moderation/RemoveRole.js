...
module.exports = class extends Command {
    ...
    async run(message, ...args) {
        ...
        if (!target.roles.cache.has(rolename)) return message.channel.send('This person does not have the role you have provided.');

        try {
            target.roles.remove(role)
            message.channel.send(`Successfully removed **${role.name}** from **${target.user.username}**.`)
        } catch(err) {
            message.channel.send('An error has occured that has prevented your command from running correctly. Double-check that the user you are trying to remove a role from has that role before contacting support.')
            console.error(err)
        }
    }
}